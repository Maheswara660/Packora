...
class AxmlRebuilder {
    ...
    fun expandAndModifyWithAliases(
        ...
        return try {
            ...
            patchTargetSdkVersion(parsed, 35)
    ...
    private fun patchTargetSdkVersion(parsed: ParsedAxml, newTargetSdk: Int) {
        ...
        if (targetSdkAttrIndex < 0) {
            AppLogger.w(TAG, "android:targetSdkVersion not found in resource map, skip patch")
            return
        }
        ...
        for (chunk in parsed.chunks) {
            ...
            for (i in 0 until attrCount) {
                ...
                if (attrName == targetSdkAttrIndex) {
                    buffer.putInt(attrOffset + 16, newTargetSdk)
                    AppLogger.d(TAG, "Patched targetSdkVersion to $newTargetSdk")
                    return
                }
            }
        }
