...
class JarSigner(private val context: Context) {
    ...
    companion object {
        ...
        private const val DEFAULT_PKCS12_FILE = "packora_keystore.p12"
    ...
    fun getSigningSchemeOptions(): SigningSchemeOptions {
        ...
        return try {
            com.google.gson.Gson().fromJson(file.readText(), SigningSchemeOptions::class.java)
    ...
    fun setSigningSchemeOptions(options: SigningSchemeOptions): SigningSchemeOptions {
        ...
        try {
            File(context.filesDir, SIGNING_SCHEME_FILE).writeText(com.google.gson.Gson().toJson(safe))
    ...
    private fun tryGenerateAndroidKeyStoreKey(): Boolean {
        return try {
            ...
            val spec = KeyGenParameterSpec.Builder(
                ...
                .setKeySize(KEY_SIZE)
                .setCertificateSubject(X500Principal("CN=Packora, O=Packora, C=IN"))
    ...
    private fun getOrCreateKeystorePassword(): CharArray {
        ...
        val keyStoreFile = File(context.filesDir, DEFAULT_PKCS12_FILE)
        val legacyPassword = "packora_sign"
        if (keyStoreFile.exists()) {
            try {
    ...
    private fun generateCertViaAndroidKeyStore(softwareKeyPair: KeyPair): X509Certificate {
        val tempAlias = "packora_cert_gen_temp_${System.currentTimeMillis()}"
    ...
    fun generateAndLoadCustomKeystore(
        outputFile: File,
        password: CharArray,
        alias: String,
        ...
        return try {
            ...
            val customFile = File(context.filesDir, CUSTOM_PKCS12_FILE)
            outputFile.copyTo(customFile, overwrite = true)

            File(context.filesDir, CUSTOM_PASSWORD_FILE).writeText(String(password))
            File(context.filesDir, CUSTOM_ALIAS_FILE).writeText(alias)
            val keypassFile = File(context.filesDir, CUSTOM_KEYPASS_FILE)
            keypassFile.writeText(String(password))

            if (loadPkcs12(customFile, password, alias, password)) {
                currentSignerType = SignerType.PKCS12_CUSTOM
                AppLogger.d(TAG, "Successfully loaded generated custom keystore")
                true
            } else {
                false
            }
        } catch (e: Exception) {
            AppLogger.e(TAG, "Failed to generate custom keystore", e)
            false
        }
    }
    ...
    private fun buildSubjectDn(spec: CertificateSpec): X500Principal {
        ...
        val dn = if (parts.isEmpty()) "CN=Packora" else parts.joinToString(", ")
    ...
    private fun generateSelfSignedCertificate(keyPair: KeyPair): X509Certificate {
        ...
        val subject = X500Principal("CN=Packora, O=Packora, C=IN")
        val serialNumber = BigInteger.valueOf(now)
    ...
    private fun trySignWithRetry(inputApk: File, outputApk: File, maxRetries: Int): Boolean {
        ...
        throw RuntimeException("Signing APK failed for %d configs. Detail: %s".format(configs.size, detail), lastException)
    }
    ...
    private fun buildManifest(digests: Map<String, String>): ByteArray {
        ...
        sb.append("Created-By: 1.0 (Packora)\r\n")
        sb.append("\r\n")
