...
Packora runs Tampermonkey/Greasemonkey-style `.user.js` scripts with a `GM_*` bridge. Import a script and it is parsed, stored, and injected like any other extension.
...
| `GM_openScriptWindow` / `GM_updateScriptWindow` / `GM_closeScriptWindow` | Packora-specific script windows. |
| `GM_info` | `{script:{name,version,description,author,namespace,grants,resources}, scriptHandler:'Packora', version:'1.0'}` |
