...
- **Device binding (one-time codes)** — remote-verification only (`activationRemoteConfig.deviceBound`). The verification request carries a device identifier and the server enforces per-code seats: each code is limited to 1 device by default (first device to activate claims the seat), so a code behaves as a one-time / single-device code. Other devices are rejected with the server's message. The claimed seat survives uninstall + reinstall on the same device. Requires a verification server that enforces binding — the [reference worker](https://github.com/Maheswara660/Packora/blob/main/examples/remote-activation-worker/README.md) does this out of the box via `maxDevices`.

## Notes

- Local verification checks codes on-device; remote verification calls your server. See the [remote activation reference](https://github.com/Maheswara660/Packora/blob/main/.github/docs/remote-activation.md).
