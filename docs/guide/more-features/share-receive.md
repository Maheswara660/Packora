...
The app dispatches a `packora:share` DOM event on `window` and `document`, and also exposes a small API:

```js
// Subscribe to share batches. Returns an unsubscribe function.
const off = window.PackoraShareInbox.onShare((items) => {
...
const pending = window.PackoraShareInbox.peek();
const taken = window.PackoraShareInbox.take();   // also marks them consumed natively
```

The `packora:share` event carries the same array as `event.detail.items`:

```js
window.addEventListener('packora:share', (e) => {
...
```js
window.PackoraShareInbox.consume(item.id);       // or consume([id1, id2, …])
```

A consumed item is never re-announced — not to a later document, not on the next app launch. An item nobody consumed still re-fires `packora:share` on each document load, so a reload in the middle of handling cannot lose it. Either way, a `fileUrl` already handed out keeps resolving until the 1-hour expiry.
